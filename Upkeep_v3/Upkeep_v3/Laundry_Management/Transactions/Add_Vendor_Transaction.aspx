<%@ Page Title="" Language="C#" MasterPageFile="~/UpkeepMaster.Master" AutoEventWireup="true" CodeBehind="Add_Vendor_Transaction.aspx.cs" Inherits="Upkeep_v3.Laundry_Management.Transactions.Add_Vendor_Transaction" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function functionCalculation() {
            ...
            var soilded_given = parseInt($("#txtSoiledGiven").val());
            var cleaned_collected = parseInt($("#txtCleanedCollected").val());
            var rewash = parseInt($("#txtRewash").val());
            var damaged = parseInt($("#txtDamaged").val());
            var closing = parseInt($("#txtClosing").val());

            var closing_value = (Opening+soilded_given + rewash) - (cleaned_collected + damaged);
    ...
    <script type="text/javascript">
        $(function () {
            $("[id*=gvItemDetails]").find("[id*=txtSoiledGiven]").keyup(function () {
                //Reference the GridView Row.
                var row = $(this).closest("tr");

                var Opening = parseInt(row.find("td").eq(1).html());

                var soilded_given = parseInt(row.find("td").eq(2).find("input").eq(0).val());
                var cleaned_collected = parseInt(row.find("td").eq(3).find("input").eq(0).val());
                var rewash = parseInt(row.find("td").eq(4).find("input").eq(0).val());
                var damaged = parseInt(row.find("td").eq(5).find("input").eq(0).val());
                var closing_value = (Opening+soilded_given + rewash) - (cleaned_collected + damaged);

                row.find("td").eq(6).find("input").eq(0).val(closing_value);

                return false;
            });

            $("[id*=gvItemDetails]").find("[id*=txtCleanedCollected]").keyup(function () {
                //Reference the GridView Row.
                var row = $(this).closest("tr");
            ...
            $("[id*=gvItemDetails]").find("[id*=txtRewash]").keyup(function () {
                //Reference the GridView Row.
                var row = $(this).closest("tr");
                ...
                <div class="m-portlet__head">
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">Add New Vendor Transaction
                            </h3>
                        </div>
                    </div>
