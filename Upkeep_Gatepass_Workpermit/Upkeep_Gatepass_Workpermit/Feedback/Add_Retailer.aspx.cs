    ...
    public partial class Add_Retailer : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataSet ds = new DataSet();
                ds = ObjUpkeepFeedback.Retailer_CRUD(store.Text.Trim(), first_name.Text.Trim(), last_name.Text.Trim(), email.Text.Trim(), Convert.ToInt64(contact.Text.Trim()), Retailer_ID, Username, Password, LoggedInUserID, Action);
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            Session["RetailerID"] = "";
                            Response.Redirect(Page.ResolveClientUrl("~/Feedback/Retailer_Master.aspx"), false);
                        }
                        else if (Status == 3)
                        {
        ...
        public void bindRetailer(int RetailerID)
        {
            ...
            {
                DataSet ds = new DataSet();
                ds = ObjUpkeepFeedback.Retailer_CRUD("", "", "", "", 0, RetailerID,"","", LoggedInUserID, "R");
        ...
        public void DeleteRetailer(int RetailerID)
        {
            ...
            {
                DataSet ds = new DataSet();
                ds = ObjUpkeepFeedback.Retailer_CRUD("", "", "", "", 0, RetailerID,"","", LoggedInUserID, "D");
